...
const { parse } = require('url'); // For parsing URLs in Next.js handler
const next = require('next'); // Import Next.js
const { Server } = require('socket.io');
const { PrismaClient } = require('@prisma/client');
const jwt = require('jsonwebtoken');
const mediasoupService = require('./mediasoup-service'); // Assuming this is in the same dir or adjust path

const prisma = new PrismaClient();

const dev = process.env.NODE_ENV !== 'production';
const hostname = '0.0.0.0'; // Listen on all interfaces
const port = process.env.PORT || 3000; // Use a single port for both Next.js and Socket.io
...
app.prepare().then(() => {
  // Create HTTP server and attach Next.js handler
  const httpServer = createServer((req, res) => {
    // Parse URL for Next.js
    const parsedUrl = parse(req.url, true);
    handle(req, res, parsedUrl); // Let Next.js handle all HTTP requests
  });
  ...
  // Store active connections and meeting states (same as your code)
  ...
  // Authentication middleware (same as your code)
...
io.on('connection', async (socket) => {
  ...
  // Graceful shutdown (same as your code)
